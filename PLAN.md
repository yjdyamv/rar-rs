...
- **老版本 RAR 只读（2026-09 起）**：`Rar!\x1a\x07\x00` 容器族（RAR 1.5–4.x）读取——块扫描/文件头（unicode 名、DOS 时间、salt、字典位）、STORE 直通 + **三代解码器全覆盖**：RAR3/4（unp_ver ≥ 29）LZSS+Huffman + PPMd 变体 H + 五大标准 VM 过滤器（E8/E8E9/Itanium/Delta/RGB/Audio）、RAR 2.x（20/26，LZSS+Huffman + 音频块）、RAR 1.5（15，自适应 Huffman 老 LZ）——（`codec/rar29.rs`+`codec/ppmd.rs`+`codec/rar20.rs`+`codec/rar15.rs`，rars 解码半移植）含 RAR3/4 solid 链共享窗口 + RAR15/20/30 全代解密。对 WinRAR 5.91 `-ma4` 夹具与 rars 语料（真 RAR 3.0/2.0/2.5/1.5.4：PPMd、六种过滤器、solid-PPMd、PPMd 内嵌过滤器、音频 WAV、17 文件 doc 集、RAR20/RAR15 老密码）字节级通过（CRC 门）。WinRAR 5.x/7.x 的 RAR4 写器不再产 PPMd，老格式夹具只能来自 rars 语料（现代 WinRAR 连 RAR2.x/1.5 也不能产）。非标准（通用）VM 程序、solid RAR2.x/1.5 链、分卷（含 .r00 老命名）仍不支持。参考环境：`~/Desktop/winrar591`（WinRAR 5.91 绿色版，7-Zip ZS 提取），7.23 无 `-ma4` 故不能产 RAR4 夹具。
...
- solid RAR2.x/1.5 链：现按单成员独立解码（无链窗）；FHD_SOLID 的归档若跨成员引用会 CRC 失败——需把 rar4 链引擎泛化为 unp_ver 分派（rar15 已带 solid 参数/64 KiB 窗，只差接线）
- 分卷 RAR4（`.partN.rar` 新命名与 `.r00/.r01` 老命名）、split-before/after 成员、多卷 solid（rar154/random.rar 是拆 2 MiB 的分卷夹具，单卷不可解）
