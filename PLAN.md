...
**`-hp` 头加密编辑全补（2026-09）**：v1 的「`-hp` 拒绝」边界解除——主头是带 MHD_PASSWORD 的明文标记，`format/rar4/mod.rs::decrypt_encrypted_header` 提供内存态逐块头解密，`archive/rar4_edit.rs` 的布局扫描/注释读/`edit_rar4`/`append_prelude` 全部按口令工作：未改动块整段（含密文与盐）原样拷贝，重建/插入的块（重命名后的 FILE_HEAD、CMT、RR）用新盐重加密，**只加密头**（CMT 35 B、RR 54 B），载荷/标签表/奇偶区保持明文。solid repack 用同一口令 + `encrypt_headers` 重建保护（`-hp` 同时含数据加密，与官方一致），注释改由写侧队列 `emit_pending_rar4_comment` 在首成员前发射（`-hp` 下同样只加密 35 B 头）。恢复记录侧新增 `scan_protect_with_password` / `repair_legacy_archive_path_with_password`（CLI `rar r` 已接线），`-hp` 档的 rr 可定位、可重建、可修复。缺口令报 `RarError::Encrypted`。另修创建面 bug：`-hp` 下 NEWSUB 恢复记录曾整块加密（应为只加密 54 B 头，否则奇偶区失效）。测试锁定 `archive/rar4_edit.rs::hp_tests` 八例（rename/delete/comment/rr+修复/lock/append/solid repack/缺口令）
...
- **solid 归档 MT（06）**（2026-09 评估）：chunk 级并行（成员内 64 KiB 块跨线程波次）已工作——64 MiB 语料实测：text 2443→593 ms（4.1x）、dll-like 20678→5187 ms（4.0x）、x86syn 3303→847 ms（3.9x）、mixed 1119→399 ms（2.8x）（`--features parallel --solid-mt`，每个成员 16 MiB）。**剩余差距是结构性的**：non-solid 的 `add_batch_parallel` 可并行处理 4 个成员（各自独立 EncoderState），而 solid 必须串行处理成员（共享窗口语义）。64 MiB 4 成员实测：non-solid archive 343 ms (text) / 2871 ms (dll-like) vs solid MT8 593 ms / 5187 ms——差距 1.7-1.8x，来自成员级并行 vs 成员内 chunk 并行的本质差异。成员级 solid 并行需要跨成员窗口共享，无法简单引入 batch 路径；唯一可能的优化是"预合并小成员为一个大逻辑块再 chunk 并行"，但对已超 12 MiB 的成员无效。**结论**：chunk 级并行已兑现，成员级差距是 solid 格式的固有代价；不建议为此引入 seq/MT 分支分歧（已有 x86 +8.2% 先例）。`--solid-mt` 扫描已加到 perfbench（`--features parallel`）
- [x] **随机数据不可压缩开销归因（2026-09，A）**：原「~800 ms 未记账」被低估。用 `perfbench --only random --dict-log N` 扫描：64 MiB 随机 m3 在 dict 128KiB→32MiB 仅 2.3x（2289→5262 ms），最小字典仍 2.3s，说明瓶颈**不是匹配树**，而是**不可压缩数据上的最优解析 DP**（连 ≤2 字节偶然匹配都进 `matches.runs`，使 matchless 快路径 `runs.is_empty()` 不触发 → 走完整 DP）。最小字典下 m1=1278、m3=2538 ms（DP 占 ~1.26s）；默认 32MiB 字典下 m1=1788、m3=5262（DP 占 ~3.5s）。而归档路径因 STORE 预检仅 101ms——与归档相比的真实缺口是 ~5.4s @64MiB，主要由 DP over 偶然匹配贡献，字典只是次要因素。修复方向（codec 热路径行为变更，审计已推迟到破坏性版本）：①**已实现（B，2026-09）**：放宽 matchless 快路径——`collect_block_matches` 只报 ≥4 字节匹配，故把触发条件从「`runs` 为空」放宽到「最长匹配 ≤ `RELAXED_MATCHLESS_MAX_LEN`(=4，即收集器下限) 且前 2 个重复距离均为 0」。随机数据 ~960K 个 4 字节哈希碰撞使 `runs` 永非空，原严格快路径不触发；放宽后每块直接全字面量、跳过 DP。字节级一致由扩展后的 `matchless_fast_path_is_byte_identical` 锁定（含大字典随机、稀疏 4 字节重复压力语料、V50/V70、m2/3/5，全过）。效果：64 MiB 随机 m3 @32MiB 字典 5262→3260 ms（~38%），@128KiB 字典 m3 2538→1486 ms（贴近 m1=1211）。残余 ~1.5s 来自「最长 5+ 字节偶发碰撞块」仍走 DP（这些块真实匹配可能有益，阈值故保守停在 4，以免破坏可压缩数据字节级一致）；要进一步逼近归档的 101ms 需②。②**后续**：在裸 `encode` 加与归档一致的不可压缩 STORE 预检（连字面量熵编码地板 ~1.8s 也省掉）。`perfbench` 已加 `--dict-log N`（固定字典）+ 表头显示实际字典，用于复现本归因。
