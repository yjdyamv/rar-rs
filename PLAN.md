...
- [x] **随机数据不可压缩开销归因（2026-09，A）**：原「~800 ms 未记账」被低估。用 `perfbench --only random --dict-log N` 扫描：64 MiB 随机 m3 在 dict 128KiB→32MiB 仅 2.3x（2289→5262 ms），最小字典仍 2.3s，说明瓶颈**不是匹配树**，而是**不可压缩数据上的最优解析 DP**（连 ≤2 字节偶然匹配都进 `matches.runs`，使 matchless 快路径 `runs.is_empty()` 不触发 → 走完整 DP）。最小字典下 m1=1278、m3=2538 ms（DP 占 ~1.26s）；默认 32MiB 字典下 m1=1788、m3=5262（DP 占 ~3.5s）。而归档路径因 STORE 预检仅 101ms——与归档相比的真实缺口是 ~5.4s @64MiB，主要由 DP over 偶然匹配贡献，字典只是次要因素。修复方向（codec 热路径行为变更，审计已推迟到破坏性版本，**不在本次做**）：①放宽 matchless 快路径触发条件——当所有匹配 ≤2 字节（偶然匹配，最优解析永不选它替字面量）时也走全字面量快路径，随机数据可从 5.3s 降到 ~1.8s；②或在裸 `encode` 加与归档一致的不可压缩 STORE 预检。`perfbench` 已加 `--dict-log N`（固定字典）+ 表头显示实际字典，用于复现本归因。
