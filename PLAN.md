...
- [ ] **issue 15 — 借 Fast LZMA2 / zstd
      的价格驱动解析思路提速**：有意做，2026-09-18
      立项。目标是**压缩速度**，手法是「给解析器补中间档、把每位置代价从更深的搜索
      换成更省的价格计算」，默认档字节不动（比率是契约）。四个杠杆（价格分量制表、
      更便宜的可压性预检、价格驱动的 MT
      中间档、匹配器行化）、上游事实与验收判据见
      `docs/issues/compression-perf/issues/15-fl2-zstd-parser-tiers.md`。参照值：FL2
      宣称高档位快 20%–100%，代价是同字典下产物大
      1%–5%，这个取舍只允许出现在**新档**。
