...
阶段 B 进度（2026-09）：非 solid `d/u/f/a` 全部落地——`d`（`edit_rar4` 支持 delete：块拷贝跳过成员 + 既有 rr 按原强度重建；删光=抹档，与 RAR5 一致；solid 删除拒绝待阶段 C；rename+delete 同成员冲突拒绝）；`a/u/f`（RAR4 append：`prepare_append` rar4 分支门禁 + 截断尾部 NEWSUB/ENDARC 暂存前缀，关闭时按原强度重建 rr；`rar a/u/f` CLI 编排即 editor 删 + append 追加，格式随容器自动保持 RAR4）。6.23 双向 `UnRAR t` 通过（含 6.23 自建档删/追加）；**追加后重建的 rr 可被 6.23 `Rar.exe r` 修复追加成员（逐字节还原）**。solid 的 d/u/f/a 拒绝待阶段 C repack
