...
**`-hp` 头加密编辑已落地（2026-09）**：`format/rar4/mod.rs::decrypt_encrypted_header` 提供内存态头解密；`archive/rar4_edit/` 的 `scan_layout`/`read_comment`/`edit_rar4`/`append_prelude` 全部按口令工作，`recovery/legacy.rs::scan_protect_with_password` 让恢复记录在 `-hp` 档上可定位/重建（记录自身的标签表与奇偶区不加密，仍可修复）。solid repack 与 create 路径用同一口令重建保护（`-hp` 同时含数据加密，与官方一致）。写侧 `emit_pending_rar4_comment` 让注释块在 `-hp` 档里也只加密 35 字节头。**分卷编辑已落地（2026-09-11，阶段 D）**：官方 6.23 实测确认了行为边界——`rn`/`ch`/`k`/`c`/`rr` 在 RAR4 分卷集上成功，`d`/`a` 报 "Cannot modify volume"；且官方**不做卷重平衡**（改名后 vol1 长 31 字节、注释 +65，卷可超过 `-v`）。我们照此实现 `rn`/`ch` 与 `k`：逐卷独立扫描块流（每卷自己签名/主头），对名字匹配的 FILE_HEAD（含跨卷 chunk 头，它们重复成员名）用 `rename_file_header` 重建，未改动块连密文原样拷贝（`-hp` 下重建头用新盐重加密），最后经 `fs::atomic::commit_files` 整体提交换卷（失败回滚 + journal）；`k` 只补第一版主头的 LOCK 位（与官方一致）。`d`/`a` 保持拒绝，注释已支持（`CMT` 插在首卷主头后，官方同位置）；`rr` 继续拒绝（官方自身的多卷 RR 修复挂死，见上）；成员注释（`cf`）在分卷上继续拒绝（RAR4 成员注释本身也不是互操作保证，见 `PLAN.md`「已知小差异」）。
