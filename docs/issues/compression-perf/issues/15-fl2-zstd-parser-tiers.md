...
**静态价格的 cheap tier**：把 MT 低步数层的「比长度」换成「比价格」——候选取
finder 最优、最近两个距离与重复长度，按估计 bit/字节比较，再加一步价格前视（zstd
的 `opt0` 形状）。藏在 `RAR_RS_MT_PRICED` 后面测 `ntoskrnl.exe` m3/mt8：无开关
52.05%，有开关 54.45%，速度相同；把模型的字面量价格从 3 扫到 14 bit 只在
53.07%–54.45% 之间游走，
从未低于基线。原因：静态预块估计看不到「多一个符号」的表代价（逐字节比较会把长匹配
打碎成短而便宜的匹配），所以「取最长、次位置更长就
lazy」反而是更好的代理。已回退， 不留开关（同 issue 14 的负例处理），数据在
[`map.md`](../map.md)。

留下的方向是**真实价格 + 全局决策**：先跑一遍便宜 walk 统计频次，再在窗口内有界
DP （zstd 的 `opt1` 形状，每位置只用 chain 的单一候选）。`-m2` 的未定价全 DP
是便宜 walk 的 6x，窗口版是中间那个未测点。
