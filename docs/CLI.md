...
> Last verified: 2026-09-22 @ `1bffd0f`; switch coverage is checked against the
...
| `r`         |       | Repair the archive with its recovery record; with no record, reconstruct `rebuilt.<name>` from the members that still decode and verify, resyncing past corrupt RAR5 headers (single- or multi-volume, plaintext headers) and exiting 3 when a header was lost, like WinRAR |
