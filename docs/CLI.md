...
> Last verified: 2026-09-22 @ `b4b8e04`; switch coverage is checked against the
...
| `rr`        |       | Add an inline recovery record. WinRAR's `rr` ignores every requested strength and always writes its 3% default (measured: 6.23 and 7.23), so we match the default but honor an explicit one, with `a`'s units: `-rr20%` is 20 percent, `-rr20` is a legacy RAR4 parity-sector count (that percent on RAR5, whose record is percent-sized), bare `-rr` is 3%. A trailing argument (`rr archive.rar 20` / `20%`) is our own extension and always a percentage. On `a`/`update`, `-rr<N>` likewise counts legacy RAR4 parity sectors. Percent forms round **up** to whole 512-byte sectors (two-sector floor), so a record never protects less than the requested percentage. With `-v`, every data volume carries its own record protecting that volume, and `.rev` recovery volumes (`-rv`) may be combined with it. On a volume set the record's strength is carried over from the existing volumes, so an edit rebuilds them instead of dropping them |
| `rv[N]`     |       | Create `.rev` recovery volumes for an existing set (`rv3` / `rv10%`, default 10%)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `r`         |       | Repair the archive with its recovery record, reporting WinRAR's per-sector line `Sector N (offsets XXX...YYY) damaged - data recovered` (hex offsets) for every damaged sector; damage parity cannot rebuild reads `cannot recover data`, then it asks `Reconstruct archive structure ? [Y]es, [N]o` (quiet mode rebuilds without asking) and exits 3, like WinRAR. With no record it reconstructs `rebuilt.<name>` from the members that still decode and verify, resyncing past corrupt RAR5/RAR4 headers (plaintext headers only). Exit 3 whenever a member was lost or nothing was produced, 0 when everything was recovered — one rule for every container (WinRAR answers 0 for a legacy loss, a container fork we do not copy). RAR 1.3/1.4 prints the banner, says `Cannot repair archive with old format` and produces nothing (exit 3)                                                                                                       |
...
| Switch       | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-v<size>`   | Multi-volume (e.g. `-v1m` ≈ 1 MB, `-v100k` ≈ 100 KB); both RAR4 and RAR5 use WinRAR's zero-padded `partNN` names (width = digit count of the total volume count). `-v-` cancels a requested size (WinRAR), leaving a single archive                                                                                                                                                                                           |
| `-vn`        | RAR4 volume sets (`-ma4`) use the old `.rar`/`.rNN` names (and no `MHD_NEWNUMBERING`) instead of the default `.partNN.rar`; ignored by other formats (WinRAR's `-vn`)                                                                                                                                                                                                                                                         |
...
| `-rv<N\|N%>` | Recovery volumes; at creation the count is capped at the data-volume count (the standalone `rv` command at 10×). RAR4 sets (`-ma4`) use the trailer `.rev` layout (`base.partNN.rev`; `-vn` → `baseN.rev`), since our volumes end in the 20-byte `ENDARC`'s zero tail; the legacy full-parity format (`base<data>_<rec>_<idx>.rev`) is still **read** for older sets; silently skipped when the archive ends up single-volume |
| `-qo[-\|+]`  | Quick-open record; `-qo` writes it, `-qo-` disables it (our default). WinRAR's console `rar a` writes no record for a small archive but writes one once it grows (measured: none at ~4 KB, present at 8 KB), so our archives simply carry none unless `-qo` is given. Listing commands (`l`/`v`/`lt`/`lb`/`i`, and UnRAR's) read it when present and fall back to a full scan otherwise, like WinRAR                          |
...
records are rebuilt (one per volume for a volume set) and the quick-open record
is dropped.
