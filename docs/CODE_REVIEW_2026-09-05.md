...
- 状态：**已记录，当日闭环修复**（2026-09-05；见下「修复事项与状态」）。
...
1. **版本轴命名分裂**（naming coherence）——`archive/writer.rs` 字段 `format_version`→`compression`，但 `options.rs:45` 的 `CreateOptions::format_version` 仍是同一 `ArchiveVersion` 类型。ADR 0004 收敛为单一表，两个公共 API 却叫法不同。已被 ADR 0004 记录（plan:376-378），但概念应只有一个名字；见下修复事项 FX-01。**（已修复：`CreateOptions::format_version` 重命名为 `compression`；`model::FileHeader::format_version` 是 wire 容器版本 4/5，保留。）**
...
- 物理树与 Phase 5 目标布局吻合；`ArchiveVersion` 表、`CreateOptions::compression`（原名 `format_version`，已统一）、`WriterOptions::compression`、`ArchiveEntry::version()`、CLI `-ma` 映射均合 ADR 0004。
...
## 修复事项与状态（2026-09-05 当日闭环）

| # | 位置 | 处置 | 轴 | 状态 |
|---|---|---|---|---|
| FX-01 | `CreateOptions::format_version`（`options.rs:45`） | 重命名为 `compression`，与 `WriterOptions::compression` 统一；`model::FileHeader::format_version`（wire 4/5）保留；同步 ADR 0004 / plan / rar4-creation-spec / 本记录 | Standards | ✅ 已修复 |
| FX-02 | `crates/rar-napi/src/{error,lib,options,tasks}.rs` docstring | "rar5 …" → "rar-rs …"，清扫更名残留 | Standards | ✅ 已修复 |
| FX-03 | `format/rar5/extract.rs:26` | `archive_write` 别名 → `rar5_write` | Standards | ✅ 已修复 |
| FX-04 | `options.rs` + `archive/writer.rs` | 抽取 `options::require_writable_version` 共享校验，两处报错同文案 | Standards | ✅ 已修复 |
| SP-01 | `model/` 目录 | plan Phase 5 加注：目标树 `model/` 细分（path/timestamp/compression/redirect）为设想布局，现状仅 entry+chunk，不为配图造假文件 | Spec | ✅ 已修正（文档职责） |
| SP-02 | CLI `cmd_comment_write`/`cmd_repair` | plan Phase 6 加注：typed reader 尚无 comment/path 服务，`rar cw` 与 repair 校验保持 `RarArchive::open`；迁 typed 待 `ArchiveReader` 补 API | Spec | ✅ 已修正（文档职责） |
| SP-03 | ADR 0003 决策 3 | 措辞改为 "internal 为期望态，当前经 `#[doc(hidden)]` format 树可触达（项 `pub(crate)`）"；树可见性收敛留在 breaking release | Spec | ✅ 已修正（文档职责） |

## 结论

- **Standards 轴**：0 硬性违规 + 5 判断性发现；最需处理 `FX-01`（版本轴双名）——**已修复**（统一为 `compression`）。
- **Spec 轴**：2 缺失 + 0 越界 + 3 偏差；最需处理 `model/` 目标树缺口（SP-01）与 "CLI 单一 open 路径" 论断失实（SP-02）——**均已文档修正**；`rar cw`/`repair` 迁回 typed role 留待 `ArchiveReader` 补 comment/path 服务。