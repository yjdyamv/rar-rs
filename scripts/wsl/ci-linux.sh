...
#
#     1-12  lint job    fmt (workspace + fuzz), host-path guard, cargo check
#                       (workspace / no-default / wasm -D warnings / fuzz /
#                       fuzz+fuzzing), clippy -D warnings (workspace, then
#                       every rar-rs feature combination), the full workspace
#                       test suite (stale `rarfiles.lst` removed first, failing
#                       test names echoed the way CI annotates them), rustdoc
#                       -D warnings
#       13  interop job official rar 6.23 + unrar 7.23 suites, driven with
#                       SA_REQUIRE_OFFICIAL=1 so a missing tool fails loudly
#  14-18  binding job   npm ci, native addon build + test, wasm addon build,
#                       WASI loader patch + WASI-forced test
#       19  heavy only  fuzz smoke loop (CI runs it on tags/schedule only)
#
...
#
# Usage: scripts/wsl/ci-linux.sh [FROM] [TO]      (defaults: 1 18)
# Prereqs: scripts/wsl/setup-{apt,rust,node}.sh, a repo clone (REPO=~ by
#          default at ~/rar-rs) and scripts/wsl/fetch-rarlab.sh for step 13.
...
if want 11; then
  step "11/19 cargo test (workspace, all features)"
...
if want 12; then
  step "12/19 rustdoc (-D warnings)"
  RUSTDOCFLAGS="-D warnings" run cargo doc --workspace --no-deps
fi

if want 13; then
  step "13/19 official rar/unrar interop (rar 6.23 + unrar 7.23)"
...
if want 19; then
  step "19/19 fuzz smoke (CI: tags/schedule only)"
