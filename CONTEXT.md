...
- `archive/` 已拆为 facade 层（`mod.rs` 结构体/构造器/生命周期，`reader.rs`/`writer.rs`/`editor.rs` 角色门面，`create.rs` 写生命周期，`transaction/` 外科重写（`multivolume`/`edit`/`plan`/`execute`/`header` 角色模块），`entry.rs` 条目类型，`discovery.rs` 分卷发现）；`format/rar5/write/`（成员入口 / 发射 / 流式 / batch）与 `format/rar4/write/`（原语 / 编排 / CBC）已文件级拆分，格式中性的写入口在 `format/shared/write_ops.rs`；读写路径分别在 `format/rar5/extract/`（`open`/`read`/`members`/`dest`/`solid`/`decode`/`verify` 角色模块）与 `format/rar5/write/`。
