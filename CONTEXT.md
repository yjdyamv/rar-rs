...
- **Streaming payload（流式负载）** — `write_streamed_payload`（`format/rar5/write/mod.rs`）：统一流式写路径（单卷/分卷 + 可选流式 AES-256-CBC），`write_stored_file` 是其 STORE 特例。
