...
- **Legacy family（老容器族）** — `Rar!\x1a\x07\x00` 7 字节签名的 RAR 1.5–4.x 容器（`rar40/`），与 RAR5 8 字节签名区分；固定宽度头 + 16 位头 CRC（ext-time 尾不在覆盖内）、`format_version: 4`。读路径：`rar40/mod.rs` 扫描/解析（`Rar4VolumeScan` 跨卷 split 合并：每卷一个 chunk；`-hp` 主头 MHD_PASSWORD 后每块按 `[8B salt][align16(head_size) 密文]` 解密再解析，data 起点为块起始+磁盘头长 `header_end`）→ `rar40/read.rs` 成员解码门面（跨卷按 chunk 读取拼装后解密/解码）。多卷发现 `discover_volumes` 支持 `.partN.rar`（新命名）与 `.rar/.rNN`（老命名，r→z 每百卷升字母，任意卷入口）。
