...
#[cfg(feature = "parallel")]
pub(crate) fn add_batch_parallel(cx: &mut dyn Engine, entries: &[BatchEntry<'_>]) -> RarResult<()> {
    crate::format::shared::write_ops::progress_set_batch_total(cx, entries)?;
    let progress = cx.progress_slot().map(|(tracker, _)| tracker);
    let mut i = 0usize;
    while i < entries.len() {
        cx.check_cancel()?;
        ...
        if !wave.is_empty() {
            ...
            let prepared = prepare_batch_wave(cx, &wave, progress.as_ref(), threads)?;
            cx.check_cancel()?;
            for (idx, entry) in prepared {
                cx.set_progress_member(idx);
                write_prepared_entry(cx, entry)?;
            }
        }
...
fn prepare_batch_wave(
    ...
    let ctx = BatchPrepareCtx {
        ...
        dict_size_log: cx.write_ctx().compression.dict_size_log,
        dict_size_bytes: cx.write_ctx().compression.dict_size_bytes,
        force_v70: cx.write_ctx().compression.force_v70,
        filters: cx.write_ctx().compression.filters,
        save_ctime: cx.write_ctx().meta.ctime,
        save_atime: cx.write_ctx().meta.atime,
        save_mtime: cx.write_ctx().meta.mtime,
        save_owner: cx.write_ctx().meta.owner,
        time_precision_seconds: cx.write_ctx().meta.time_precision_seconds,
        threads,
        wave_len: wave.len(),
...
/// sequential encoding used by `add_file_rar5` (fresh encoder window
/// per chunk) instead of `add_bytes_rar5` (one shared window pass).
...
fn prepared_from_payload(
    ...
    let (header_crc, extra_data, stored_hash, encr) =
        super::emit::payload_extra_and_crc(ctx.password, plain_crc, plain_blake);
    ...
    let payload = super::emit::encrypt_payload_with(encr.as_ref(), &payload);
    Ok(PreparedEntry { plan, payload })
}

#[cfg(feature = "parallel")]
fn write_prepared_entry(cx: &mut dyn Engine, entry: PreparedEntry) -> RarResult<()> {
    ...
    if let Some((progress, member)) = cx.progress_slot() {
    ...
    super::emit::write_file_entry(cx, &entry.plan, &entry.payload)
}
