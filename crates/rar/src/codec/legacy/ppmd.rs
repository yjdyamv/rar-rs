...
/// Codec error mirroring the rars `codec::Error` variants this decoder uses.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum Error {
    /// Structurally invalid PPMd stream data.
    InvalidData(&'static str),
    /// The packed stream ended before the range coder needed another byte.
    NeedMoreInput,
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::NeedMoreInput => f.write_str("PPMd stream is truncated"),
        }
    }
}
